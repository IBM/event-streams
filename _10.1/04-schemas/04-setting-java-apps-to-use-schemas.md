...
11. If your project uses Maven, add the following dependency to your project `pom.xml` file to use the {{site.data.reuse.short_name}} schema registry `serdes` library:
...
The code snippet from the **Imports** section includes Java imports to paste into your class, and sets up the application to use the {{site.data.reuse.short_name}} schema registry's `serdes` library, for example:
...
Kafka Streams applications can also use the {{site.data.reuse.short_name}} `serdes` library to serialize and deserialize messages. For example:
