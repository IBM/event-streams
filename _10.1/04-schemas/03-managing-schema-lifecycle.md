...
Java producers using the {{site.data.reuse.short_name}} schema registry `serdes` library will throw a `SchemaDisabledException` when attempting to produce messages using a disabled schema version.
...
If a schema is no longer available in the registry, Java applications using the {{site.data.reuse.short_name}} schema registry `serdes` library will receive a `SchemaNotFoundException` message when they attempt to use the schema.
