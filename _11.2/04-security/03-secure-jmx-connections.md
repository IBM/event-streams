...
Metrics can be retrieved from applications running inside your Kubernetes cluster by connecting to an exposed JMX port. The metrics can also be pushed in various formats to remote sinks inside or outside of the cluster by using JmxTrans.
...
2. Run the following commands:
   ...
   `kubectl get secret <cluster-name>-jmx -o jsonpath='{.data.jmx\-username}' -namespace <name_of_your_namespace> | base64 -decode > jmx_username.txt`

   `kubectl get secret <cluster-name>-jmx -o jsonpath='{.data.jmx\-password}' -namespace <name_of_your_namespace> | base64 -decode > jmx_password.txt`
...
```
kubectl -n <instance_namespace> get secret {{site.data.reuse.jmx-secret}} -o yaml --export | kubectl -n <application_namespace> apply -f -
```
