...
As explained in the [Managing access](../../security/managing-access) section, Salted Challenge Response Authentication Mechanism (SCRAM)  or the {{site.data.reuse.icpfs}} Identity and Access Management (IAM) is used to access the {{site.data.reuse.short_name}} UI.
...
You can configure SCRAM authentication for the {{site.data.reuse.short_name}} UI, as described in [configuring UI security](../configuring/#configuring-ui-security). Kafka users that have been configured to use SCRAM-SHA-512 authentication can log into the UI and access components based on the ACL permissions of the user.
