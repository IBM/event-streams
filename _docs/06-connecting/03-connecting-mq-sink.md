...
![Event Streams 2018.3.1 and later icon](../../../images/2018.3.1.svg "Only in Event Streams 2018.3.1 and later.") {{site.data.reuse.kafka-connect-mq-sink}} is supported in {{site.data.reuse.long_name}} 2018.3.1 and later.
...
The queue manager is now ready to accept connection from the connector and get messages from a queue called `MYQSINK`.
...
To test the connector you will need an application to produce events to the `TSINK` topic.
...
1. Select **Choose existing topic** and choose `TSINK`
1. Click **Generate**
